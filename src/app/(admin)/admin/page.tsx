...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        setDots(rows.filter(r => r.lat && r.lng).map(r => ({
          id: r.id, name: r.name, plan: r.plan,
          is_active: r.is_active, lat: r.lat!, lng: r.lng!,
        })));
        setLoading(false);
      });
  }, []);
  ...
  const kpiCards = [
    ...
    { label: 'MRR',
      value: kpis ? `$${kpis.mrr.toLocaleString('es-MX')}` : '—', icon: '💰' },
  ];
    ...
    <div>
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', marginTop: '4px' }}>
          Visibilidad global de todos los restaurantes registrados.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '28px' }}>
        {kpiCards.map(k => (
          <div key={k.label} style={{ backgroundColor: '#1a2535', border: '1px solid #1e2d3d', borderRadius: '12px', padding: '18px' }}>
            ...
            <div style={{ fontSize: '26px', fontWeight: 700, color: '#f59e0b', fontFamily: 'monospace' }}>
              {loading ? '…' : k.value}
            </div>
            <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)', marginTop: '3px' }}>{k.label}</div>
          </div>
        ))}
      </div>

      {/* Heatmap */}
      <div style={{ backgroundColor: '#1a2535', border: '1px solid #1e2d3d', borderRadius: '14px', padding: '20px', marginBottom: '28px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
          <div>
            <span style={{ fontWeight: 600, fontSize: '14px', color: '#f1f5f9' }}>Distribución geográfica</span>
            <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', marginLeft: '10px' }}>
              {dots.length} con coordenadas
            </span>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            {Object.entries(PLAN_COLOR).map(([plan, color]) => (
              <span key={plan} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: 'rgba(255,255,255,0.5)' }}>
        ...
        <div style={{ position: 'relative' }}>
          <svg viewBox="0 0 640 320" width="100%" style={{ display: 'block', borderRadius: '8px', background: '#0d1720' }}>
            {/* Simplified world outline — just continents as a light grid reference */}
            <rect x="0" y="0" width="640" height="320" fill="#0d1720" />
            {/* Grid lines */}
          ...
          {dots.length === 0 && !loading && (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.25)', fontSize: '13px' }}>
              Sin tenants con coordenadas registradas aún
            </div>
          )}
        </div>
      </div>
